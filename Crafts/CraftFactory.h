...
public:
	...
	static Craft* getNewCraft(std::string ID) {return craftMap.find(ID)->second();}
	static std::map<std::string, std::function<Craft*()>> getCraftMap() {return craftMap;}
	static void regCraft(std::string ID, std::function<Craft*()> f) {craftMap[ID] = f;}
