...
void AutumnLeafCutter::displayHitDescription(Entity* hitter, Entity* hittee,
		int dmg) {
	std::cout << hitter->getName()
			<< " swung his sword around pretty well and hit "
			<< hittee->getName() << " real good for " << dmg << " damage.\n";
}

bool AutumnLeafCutter::displayAfterHitDescription(Entity* hitter) {
	std::cout << hitter->getName() << " has been moved to "
			<< hitter->getPositions()[0] + 1 << "-"
			<< hitter->getPositions()[1] + 1 << " in the process.";
	return true;
}
...
void AutumnLeafCutter::moveOpposite(Entity* entity, Position aimed) {
	...
	if (entity->getPositions()[0] > aimed[0]) {
		moveTo[0] = entity->getPositions()[0]
				- 2 * entity->getXYdistanceToPoint(aimed)[0];
	...
	if (entity->getPositions()[1] > aimed[1]) {
		moveTo[1] = entity->getPositions()[1]
				- 2 * entity->getXYdistanceToPoint(aimed)[1];
	} else if (entity->getPositions()[0] < aimed[0]) {
		moveTo[1] = entity->getPositions()[1]
				+ 2 * entity->getXYdistanceToPoint(aimed)[1];
	} else {
