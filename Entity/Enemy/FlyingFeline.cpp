...
FlyingFeline::FlyingFeline(Position position) :
		Enemy(baseStats, name, position, efficacy, representation) {
...
void FlyingFeline::takeTurn() {
	...
	if (getDistanceToPoint(
			findClosestPositionToPointInRange(closestCharacter->getPositions()))
			>= getRNG()) {
		if (closestCharacter->getPositions()[0] > getPositions()[0]
				&& abs(closestCharacter->getPositions()[0] - getPositions()[0])
						> 1) {
			moveEntity( { getPositions()[0] + getTotalStats()[9],
					getPositions()[1] });
		} else if (abs(closestCharacter->getPositions()[0] - getPositions()[0])
				> 1) {
			moveEntity( { getPositions()[0] - getTotalStats()[9],
					getPositions()[1] });
		} else if (closestCharacter->getPositions()[1] > getPositions()[1]) {
			moveEntity(
					{ getPositions()[0], getPositions()[1] + getTotalStats()[9] });
		} else {
			moveEntity(
					{ getPositions()[0], getPositions()[1] - getTotalStats()[9] });
		}
		Map::update();
		Map::display();
		std::cout << getName() << " moved to " << getPositions()[0] + 1 << "-"
				<< getPositions()[1] + 1 << std::endl;
		enterPause();
	} else {
