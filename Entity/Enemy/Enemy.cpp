...
void Enemy::moveEntity(Position positions) {
	if (getDistanceToPoint(positions) > getSPD()) {
		std::cout << "This is an error, please tell someone that " << getName()
				<< " has a SPD/distance issue.\n\n";
		return;
	}

	for (int i = 0; i < (signed) getEntities().size(); i++) {
		if (getEntities()[i]->getPositions()[0] == positions[0]
				&& getEntities()[i]->getPositions()[1] == positions[1]) {
			return;
		}
	}
...
void Enemy::useCraft(Craft* usedCraft, Entity* character, bool followUpAttack) {
	...
	if (!isEnemy(character)) {
		if (!isHit(character)) {
			std::cout << this->getName() << " tried to use "
					<< usedCraft->getName()
	...
	std::cout << this->getName() << " used " << usedCraft->getName() << "."
			<< std::endl;
	std::cout << "It did " << damageDealt << " damage to "
			<< character->getName() << ".\n\nPress enter to continue."
			<< std::endl;
	getline(std::cin, nice);
