...
void Character::updateArts_MasterQuartz() {
	...
	for(int i = 0; i < (signed)getMasterQuartz()->getArts().size(); i++) {
		for(int j = 0; j < (signed)getArts().size(); j++) {
			if(getMasterQuartz()->getArts()[i]->getName() == getArts()[j]->getName()) {
				added = true;
			}
		}
		if(!added) {
...
void Character::useCraft(Craft* usedCraft) {
	...
	} else if(usedCraft->getTargetting() == 1 || usedCraft->getTargetting() == 2) {
		tar = inputPositions();
	...
	} else if(getDistanceToPoint(findClosestPositionToPointInRange(tar)) <= getMOV()) {

		std::cout << getDistanceToPoint(findClosestPositionToPointInRange(tar)) << " " << getMOV();
		std::cin.get();
	...
	if(usedCraft->getTargetting() == 4 || usedCraft->getTargetting() == 5 || usedCraft->getTargetting() == 6) {
	...
	for(int i = 0; i < (signed)appliedTo.size(); i++) {
		for(int j = 0; j < (signed)effected.size(); j++) {
			if((appliedTo[i]->getPositions()[0]) == effected[j][0] && (appliedTo[i]->getPositions()[1]) == effected[j][1]) {
...
void Character::useCraft(Craft* usedCraft, Entity* enemy, bool followUpAttack) {
	...
	if(isUnbalanced(enemy, usedCraft) && !followUpAttack && !isCharacter(enemy)) {
		std::cout << "\nYou have unbalanced " << enemy->getName() << ". Your link gets to attack!\n";
...
void Character::useArt(Art* usedArt) {
	if(getCastingArt() == NULL) {
		...
		if(getEP() < usedArt->getEPCost()) {
			std::cout << "You do not have enough EP to cast " << usedArt->getName() << std::endl;
			takeTurn();
		}
		...
		if(usedArt->getTargetting() == 0) {
			ene = inputSelectEntity();
		} else if(usedArt->getTargetting() == 1 || usedArt->getTargetting() == 2) {
			tar = inputPositions();
		...
		if(ene != NULL) {
			...
			std::cout << "You have started casting " << usedArt->getName() << ", targetting " << ene->getName() << std::endl;
		} else {
			usedArt->setCasting(tar);
			std::cout << "You have started casting " << usedArt->getName() << ", at the position " << tar[0]+1 << "-" << tar[1]+1 << std::endl;
		}
		setCastingArt(usedArt);

		std::cout << "\n" << usedArt->getName() << " takes " << usedArt->getCastTime() << " Action Ticks to cast.\n\n";
		enterPause();
		return;
	}
	...
	std::vector<Position> effected = AoE(usedArt->getAimedPosition(), usedArt->getAoESize());

	std::cout << "You have casted " << usedArt->getName() << " at " << usedArt->getAimedPosition()[0]+1 << "-" << usedArt->getAimedPosition()[1]+1 << std::endl;
	enterPause();

	for(int i = 0; i < (signed)getEntities().size(); i++) {
		for(int j = 0; j < (signed)effected.size(); j++) {
			if(getEntities()[i]->getPositions()[0] == effected[j][0] && getEntities()[i]->getPositions()[1] == effected[j][1]) {
...
void Character::useArt(Art* usedArt, Entity* enemy) {
	...
	std::cout << enemy->getName() << " has " << enemy->getHP() << " health remaining!\n\n";
	enterPause();
}
...
void Character::takeTurn() {
	...
	std::string stattypes[11] = {"HP",   "EP",   "STR",   "DEF",  "ATS",   "ADF",   "SPD",   "DEX",   "AGL",   "MOV",   "RNG"};
	for(int i = 0; i < 11; i += 1) {
		if(i < 2) {
			std::cout << stattypes[i] << "    " << getTotalStats()[i] << std::endl;
		} else if(i == 2) {
...
void Character::takeTurn_Craft() {
	...
	std::cout << "Select a Craft by typing its name or selecting its number:" << std::endl;

	std::cout << std::endl;
	for(int i = 0; i < (signed)getCrafts().size(); i++) {
		std::cout << "(" << i + 1 << ")\t" << getCrafts()[i]->getName() << std::endl;
...
void Character::takeTurn_Art() {
	...
	std::cout << "Select an Art by typing its name or selecting its number:" << std::endl;

	std::cout << std::endl;
	for(int i = 0; i < (signed)getArts().size(); i++) {
		std::cout << "(" <<  i + 1 << ")\t" << getArts()[i]->getName() << std::endl;
	}
...
void Character::move() {
	...
	std::cout << "\n\nYour current position is: X = " << getPositions()[0] + 1 << ", Y = " << getPositions()[1] + 1;
...
void Character::takeTurn_Link() {
	...
	std::cout << "You are currently linked to " << getLink()->getName() << std::endl;
	...
	if(input == "2") {
		...
		for(int i = 0; i < (signed)getCharacters().size(); i++) {
			if(getCharacters()[i] != this) {
				std::cout << "(" << i+1 << ") " << getCharacters()[i]->getName() << std::endl;
			}
		}
	}
	getline(std::cin, input);
...
void Character::moveAttack(Craft* usedCraft, Position pos) {
	...
	for(int i = 0; i < (signed)attackPoints.size(); i++) {
		for(int j = 0; j < (signed)getEntities().size(); j++) {
			if(attackPoints[i][0] == getEntities()[j]->getPositions()[0] && attackPoints[i][1] == getEntities()[j]->getPositions()[1]) {
	...
	AoE(index, 0, true);
	std::cout << "If you move, you can still use " << usedCraft->getName() << std::endl;
