...
Game::Game() {
	...
	for (int i = 0; i < 5; i++) {
		new FlyingFeline( { rand() % 17, rand() % 17 });
	}
...
void Game::gameLoop() {
	...
	std::string lose;
	std::cout
			<< "When the game begins, you are linked to yourself. It is encouraged to relink on your first turn.\n\nPress enter to continue.";
	...
	while (true) {
		...
		for (int i = 0; i < (signed) turnOrder.size(); i++) {
			std::cout << "(" << i + 1 << ")\t" << turnOrder[i]->getName()
					<< "\t(" << turnOrder[i]->getRepresentation() << ")\t"
					<< turnOrder[i]->getPositions()[0] + 1 << "-"
					<< turnOrder[i]->getPositions()[1] + 1 << std::endl;
		}
		enterPause();
		...
		if (Entity::getCharacters().size() == 0) {
			...
			p->setCost(10000);
			return;
		} else if (Entity::getEnemies().size() == 0) {
			p->setCost(-10000);
			return;
		} else if (counter >= 50) {
			p->computeCost();
...
Entity* Game::nextEntityTurn() {
	...
	std::sort(turnOrder.begin(), turnOrder.end(),
			[](Entity* a, Entity* b) {return a->getActionTicks() < b->getActionTicks();});

	while (true) {
		for (int entity = 0; entity < (signed) turnOrder.size(); entity++) {
			if (turnOrder[entity]->getCastingArt() != NULL
					&& turnOrder[entity]->getCastingArt()->castTick()) {
				turnOrder[entity]->useArt(turnOrder[entity]->getCastingArt());
			}
			if (turnOrder[entity]->getActionTicks()
					<= 0&& turnOrder[entity]->getCastingArt() == NULL) {
				entityTurn = turnOrder[entity];

				if (entity != (signed) turnOrder.size() - 1) {
					turnOrder[entity]->setActionTicks(
							1000
									* (1
											- ((turnOrder[entity]->getTotalStats()[6]
													+ Dice::rollDx(1, 20, 0))
													/ 100.0)));
				} else if (initiated) {
					turnOrder[entity]->setActionTicks(
			...
			turnOrder[entity]->setActionTicks(
					turnOrder[entity]->getActionTicks() - 10);
		}
	}
}
