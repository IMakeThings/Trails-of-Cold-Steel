...
std::vector<Position> AoE(Position startingPosition, int size, bool showAoE = false) {
	...
	for(int i = 0; i <= size; i++) {
		for(int j = 0; j <= size; j++) {
			if(abs(i+j) <= size + ceil(size/3.0) && abs(i+j) != 0) {
				effectedPositions.push_back({startingPosition[0] + i, startingPosition[1] + j});
				effectedPositions.push_back({startingPosition[0] - i, startingPosition[1] - j});
	...
	for(int i = effectedPositions.size() - 1; i > 0; i--) {
		if(effectedPositions[i][0] > 16 || effectedPositions[i][0] < 0 || effectedPositions[i][1] > 16 || effectedPositions[i][1] < 0) {
