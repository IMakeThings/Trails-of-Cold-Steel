...
Entity* inputSelectEntity(int which = 0) {
	...
	std::cout << "Please select an entity by inputting it's corresponding number.\n";
	if(which == 0) {
		...
		for(int i = 0; i < (signed)enemies.size(); i++) {
			std::cout << "(" << i+1 << ")\t" << enemies[i]->getPositions()[0]+1 << "-" << enemies[i]->getPositions()[1]+1 << "\t\t" << enemies[i]->getName()  << std::endl;
		}
	} else if(which == 1) {
		for(int i = 0; i < (signed)characters.size(); i++) {
			std::cout << "(" << i+1 << ")\t" << characters[i]->getName() << "\t\t" << characters[i]->getPositions()[0]+1 << "-" << characters[i]->getPositions()[1]+1 << std::endl;
		}
	} else if (which == 2) {
		for(int i = 0; i < (signed)Entity::getEntities().size(); i++) {
			std::cout << "(" << i+1 << ")\t" << Entity::getEntities()[i]->getName() << "\t\t" << Entity::getEntities()[i]->getPositions()[0]+1 << "-" << Entity::getEntities()[i]->getPositions()[1]+1 << std::endl;
		}
	} else {
